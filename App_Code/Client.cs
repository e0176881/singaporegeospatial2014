...
namespace TwitterApiClient
{
    ...
    public class Client
    {
        ...
        public string GetUserTimelineJson(string bearerToken, string screenName, int count = 40,
        ...
        /// <returns>Raw JSON response from API.</returns>
        public string getSearchLocation(string bearerToken, string parameters)
        {
            var webrequest = CreateRequest2("/1.1/statuses/filter.json" + "?track=twitter");
            webrequest.Headers.Add("Authorization", "Bearer " + bearerToken);
            webrequest.Method = WebRequestMethods.Http.Get;
        ...
        /// <summary>
        /// Gets latest trend from a place.
        /// * API endpoint search: https://api.twitter.com/1.1/trends/place.json?id=1
        /// </summary>
        /// <param name="bearerToken">Token obtained from authentication.</param>
        /// <param name="parameters">Search parameters in raw query string format, 
        /// e.g. "q=from:BritishVogue, e.g. "q=#FNO".</param>
        /// <returns>Raw JSON response from API.</returns>
        public string GetTrend(string bearerToken, string parameters)
        {
            var webrequest = CreateRequest("/1.1/trends/place.json" + parameters);
            webrequest.Headers.Add("Authorization", "Bearer " + bearerToken);
            webrequest.Method = WebRequestMethods.Http.Get;
        ...
        private static WebRequest CreateRequest(string url)
        {
            ...
            ((HttpWebRequest)webrequest).UserAgent = "localhost:2848";
            return webrequest;
        }
        private static WebRequest CreateRequest2(string url)
        {
            var webrequest = WebRequest.Create("https://stream.twitter.com" + url);
            ((HttpWebRequest)webrequest).UserAgent = "localhost:2848";
            return webrequest;
        }
